...
const complex = [
  T("complex", "division-general", "Komplexe Division", (rng, difficulty) => {
    ...
    return makeTask({
      ...
      input: expressionInput("a + bi"), answer: { type: "complex", re, im },
  ...
  }, { levels: ["standard", "plus", "transfer"], requiredSymbols: ["i", "fraction", "minus"] }),
  ...
  }, { levels: ["standard", "plus", "transfer"], requiredSymbols: ["i", "pi", "sqrt"] }),
  T("complex", "demoivre", "de Moivre", (rng, difficulty) => {
    ...
    return makeTask({
      ...
      input: expressionInput("Komplexe Zahl"), answer: { type: "complex", expression: value },
  ...
  }, { levels: ["plus", "transfer"], requiredSymbols: ["i", "pi", "power", "sqrt"] }),
  T("complex", "roots", "Komplexe Wurzeln", (rng, difficulty) => {
    const negative = rng.pick([1, 4, 9, 16, 25, 36, 49, 64, 81, 100, 121, 144]);
    const root = Math.sqrt(negative);
    return makeTask({
      topic: "complex", id: "complex.roots", title: "Komplexe Wurzeln", difficulty,
      prompt: context("Bestimme die vollständige Lösungsmenge.") + math(`z² = −${negative}`),
      input: setInput("z₁; z₂"), answer: { type: "set", values: [`${root}*i`, `-${root}*i`] },
      explanation: `z²=${negative}i²; daher <strong>z=${root}i oder z=−${root}i</strong>.`,
      hints: ["Nutze i²=−1.", `Schreibe −${negative} als ${negative}i².`, "Eine quadratische Gleichung hat hier zwei entgegengesetzte Lösungen."],
      complexity: 6, signature: String(negative)
    });
  }, { levels: ["standard", "plus"] })
];
...
const derivatives = [
  ...
  }, { levels: ["plus", "transfer"], requiredSymbols: ["sin", "cos", "e", "power"] }),
  T("derivatives", "chain-error", "Fehleranalyse Kettenregel", (rng, difficulty) => {
    ...
    return makeTask({
      ...
      complexity: 5, signature: `${a}:${n}:${rng.int(0, 99999)}`
    });
  }, { levels: ["basis", "standard", "plus"] })
];
  ...
  T("integrals", "partial-fractions", "Partialbruchzerlegung", (rng, difficulty) => {
    ...
    return makeTask({
      ...
      steps: [
        ...
        { id: "result", prompt: "Setze die Koeffizienten in die Zerlegung ein.", inputSpec: expressionInput("Zerlegung"), answer: { type: "expression", value: finalExpression }, explanation: `Die Zerlegung lautet ${finalExpression}.`, chipLabel: "Partialbrüche", chipValue: finalExpression }
      ],
      complexity: 8, signature: `${p}:${q}:${A}:${B}`
    });
  }, { levels: ["standard", "plus", "transfer"], taskModes: ["step", "head"], requiredSymbols: ["fraction", "variables"] })
];
...
const taylor = [
  ...
  }, { levels: ["plus", "transfer"], taskModes: ["step", "head"], requiredSymbols: ["e", "power", "fraction"] }),
...
const linearSystems = [
  T("linearSystems", "classification", "LGS klassifizieren", (rng, difficulty) => {
    ...
    return makeTask({
      ...
      complexity: 6, signature: `${a}:${b}:${factor}:${right}:${secondRight}`
    });
  }, { levels: ["basis", "standard", "plus"] }),
  ...
  }, { levels: ["standard", "plus", "transfer"], taskModes: ["step", "head"] })
];
